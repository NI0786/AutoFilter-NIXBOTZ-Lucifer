...
async def pm_text(bot, message):
    ...
    await message.reply_text("<b>ʜᴇʏ {user} ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ ᴍᴏᴠɪᴇs ɢʀᴏᴜᴘ ʙʏ ᴄʟɪᴄᴋ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴ ↓↓</b>")
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✧ ᴛᴀᴋᴇ ᴍᴏᴠɪᴇ ꜰʀᴏᴍ ʜᴇʀᴇ ✧", url=f"https://t.me/Ni_Movie_Request_Group")]])
    )
        ...
        if settings['auto_delete']:
            btn.insert(0, 
                [
                    ...
                    InlineKeyboardButton("𝚀ᴜᴀʟɪᴛʏ", callback_data=f"quality#{req}"),
...
async def select_language(bot, query):
    ...
    btn = [[
        ...
        InlineKeyboardButton("Bᴇɴɢᴀʟɪ", callback_data=f"lang#{userid}#ben"),
...
async def select_quality(bot, query):
    ...
    if int(userid) not in [query.from_user.id, 0]:
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    btn = [[
        InlineKeyboardButton("Sᴇʟᴇᴄᴛ Yᴏᴜʀ Dᴇꜱɪʀᴇᴅ Qᴜᴀʟɪᴛʏ ↓", callback_data=f"lusifilms#{userid}#unknown")
    ],[
        InlineKeyboardButton("𝟹𝟼𝟶ᴘ", callback_data=f"lusifilms#{userid}#360p"),
        InlineKeyboardButton("𝟺𝟾𝟶ᴘ", callback_data=f"lusifilms#{userid}#480p")
    ],[
        InlineKeyboardButton("𝟻𝟺𝟶ᴘ", callback_data=f"lusifilms#{userid}#540p"),
        ...
        InlineKeyboardButton("𝟷𝟶𝟾𝟶ᴘ 𝙷𝚀", callback_data=f"lusifilms#{userid}#1080p HQ")
    ],[
...
async def cb_handler(client: Client, query: CallbackQuery):
            ...
            if AUTH_CHANNEL and not len(links) == 0:
                ...
                if clicked == typed:
                    if IS_VERIFY and not await check_verification(client, query.from_user.id):
                        btn = [[
                            InlineKeyboardButton("📁 vᴇʀɪғʏ ♻️", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start=", file_id)),
            InlineKeyboardButton("⚡ ʜᴏᴡ ᴛᴏ vᴇʀɪғʏ ⚡", url=HOW_TO_VERIFY)
      ] , [
    ...
    # Don't change anything without contacting @IM_NISHANTT
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_text("ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ ᴛʜᴇ <b>ʟᴜᴄɪғᴇʀ</b> ʙᴏᴛ")
        ...
        await m.edit_text("🦋")
        await asyncio.sleep(0.5)
        await m.edit_text("<b>sᴛᴀʀᴛɪɴɢ ʙᴏᴛ...</b>")
        await asyncio.sleep(0.4)
        await m.delete()        
        ...
        buttons = [[
            InlineKeyboardButton('• ᴀᴅᴍɪɴ ᴄᴏᴍᴍᴀɴᴅꜱ •', callback_data='adminxcmd'),
        ], [
            ...
            InlineKeyboardButton('cooʟ ꜰᴏɴᴛs', callback_data='font')
        ], [
        ...
        buttons = [[
            ...
            InlineKeyboardButton("👻 ɪᴛ's ᴍᴇ 😜", url="https://t.me/{temp.U_NAME}")
       ],[ 
            InlineKeyboardButton('⋞ ʙᴀᴄᴋ', callback_data='start')
       ]]
        await client.edit_message_media(
        ...
        btn = [[
            InlineKeyboardButton("⋞ ʙᴀᴄᴋ", callback_data="help"),
            InlineKeyboardButton('メ ᴄʟosᴇ メ', callback_data='close_data')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
    ...
    elif query.data == "adminxcmd":
      if query.from_user.id not in ADMINS:
       return await query.answer("⚠️ Tʜᴀᴛ's ɴᴏᴛ ғᴏʀ ʏᴏᴜ ʙʀᴏ!", show_alert=True)
        buttons = [[
...
#        totalsec = await Media2.count_documents()
   ...
   #     used_dbSize2 = (stats2['dataSize']/(1024*1024))+(stats2['indexSize']/(1024*1024))
      #  free_dbSize2 = 512-used_dbSize2
        await query.message.edit_text(
            text=script.STATUS_TXT.format( users, chats, totalp, round(used_dbSize, 2), round(free_dbSize, 2)),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
   ...
   #     free_dbSize2 = 512-used_dbSize2
        await query.message.edit_text(
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="t.me/IM_NISHANTT")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
    ...
    if not spoll:
        ...
        if not files:
            if settings["spell_check"]:
                ...
                if is_misspelled:
                 await ai_sts.edit(f'<b>ʏᴏᴜʀ ꜱᴘᴇʟʟɪɴɢ ɪꜱ ᴡʀᴏɴɢ ɴᴏᴡ ɪ ᴀᴍ ꜱᴇᴀʀᴄʜɪɴɢ ᴡɪᴛʜ ᴄᴏʀʀᴇᴄᴛ ꜱᴘᴇʟʟɪɴɢ - <code>{is_misspelled}</code></b>')
